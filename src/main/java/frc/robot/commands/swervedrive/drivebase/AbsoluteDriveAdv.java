...
public class AbsoluteDriveAdv extends Command {
   ...
   */
  public AbsoluteDriveAdv(SwerveSubsystem swerve, DoubleSupplier vX, DoubleSupplier vY,
      DoubleSupplier headingAdjust, BooleanSupplier lookAway, BooleanSupplier lookTowards,
  ...
  public void execute() {
    ...
    if (headingX == 0 && headingY == 0 && Math.abs(headingAdjust.getAsDouble()) > 0) {
      ...
      swerve.drive(translation,
          (Constants.OperatorConstants.TURN_CONSTANT * -headingAdjust.getAsDouble()), true);
    } else {
